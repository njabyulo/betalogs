...
export const SCreateMetadataRegistryEntry = z
    ...
    .refine(
        ...
        {
            message: 'promoteTo must match the type (number->meta_num, date->meta_date, boolean->meta_bool, keyword->meta_kw, text->meta_text)',
        }
    )
    .strict()
...
export type TCreateMetadataRegistryEntry = z.infer<typeof SCreateMetadataRegistryEntry>
...
export function safeValidateCreateMetadataRegistryEntry(
    data: unknown
): { success: true; data: TCreateMetadataRegistryEntry } | { success: false; error: z.ZodError } {
