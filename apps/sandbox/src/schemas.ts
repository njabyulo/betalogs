    ...
    if (schemaName === 'story') {
        const example: z.infer<typeof StoryOutputSchema> = {
            story: {
                ...
                timeline: [
                    {
                        ...
                        outcome: 'failure',
                        message: 'Checkout failed for user alice@example.com. Payment provider timeout after 3 attempts.',
                        citations: ['[id: 1]'],
                    },
                ],
                rawEvents: [
                ...
                summary: 'The checkout process for request req_abc123 failed due to a payment provider timeout. [id: 1]',
                impact: 'The user alice@example.com experienced a failed checkout, potentially leading to a lost sale. [id: 1]',
