...
function decodeQueryString(queryString: string): IQueryParams {
  try {
    ...
    throw new Error(`Invalid query string: ${error instanceof Error ? error.message : String(error)}`)
  }
}
  ...
  try {
    ...
    const response: IActivityLogResponse = {
      ...
      pagination: {
        ...
        pageSize,
        hasMore,
        ...(hasMore && { nextCursor: Buffer.from(JSON.stringify({ page: page + 1 })).toString('base64') }),
      },
      cache: {
        etag,
